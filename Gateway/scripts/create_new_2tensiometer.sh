...
# Ex: create_new_2tensiometer.sh 1 --no-init --dev-eui AC1F09FFFE12DA3F
...
DEFAULT_2TENSIOMETER_NAME="2TENSIOMETER_${1}"
...
if [[ -n $DEV_EUI ]]; then
  ...
  DEFAULT_2TENSIOMETER_YAML_FILE=${DEFAULT_2TENSIOMETER_NAME,,}_${DEV_EUI,,}.yaml
elif [[ -n $DEV_FULL_ADDR ]]; then
  echo "Device address: $DEV_FULL_ADDR"
  DEFAULT_2TENSIOMETER_YAML_FILE=${DEFAULT_2TENSIOMETER_NAME,,}_${DEV_FULL_ADDR,,}.yaml
else
  DEV_ADDR="$2"
  echo "Device address: $DEV_ADDR"
  DEFAULT_2TENSIOMETER_YAML_FILE=${DEFAULT_2TENSIOMETER_NAME,,}_${DEV_ADDR,,}.yaml
fi

echo "Optional device id: $OPT_DEV_ID"
echo "Optional init value: $OPT_NO_INIT"
echo ${DEFAULT_2TENSIOMETER_NAME}
echo ${DEFAULT_2TENSIOMETER_YAML_FILE}
...
if [[ -n $DEV_EUI ]]; then
  ...
  echo "--> calling create_full_2tensiometer_device_with_dev_addr.sh ${DEFAULT_2TENSIOMETER_NAME} $OPT_DEV_ID $OPT_NO_INIT $OPT_DEV_EUI"
  /home/pi/scripts/create_full_2tensiometer_device_with_dev_addr.sh ${DEFAULT_2TENSIOMETER_NAME} $OPT_DEV_ID $OPT_NO_INIT $OPT_DEV_EUI
else
  ...
  if [[ -n $DEV_FULL_ADDR ]]; then
    echo "--> calling create_full_2tensiometer_device_with_dev_addr.sh ${DEFAULT_2TENSIOMETER_NAME} $OPT_DEV_ID $OPT_NO_INIT $OPT_DEV_FULL_ADDR $OPT_APPSKEY $OPT_NWKSKEY"
    /home/pi/scripts/create_full_2tensiometer_device_with_dev_addr.sh ${DEFAULT_2TENSIOMETER_NAME} $OPT_DEV_ID $OPT_NO_INIT $OPT_DEV_FULL_ADDR $OPT_APPSKEY $OPT_NWKSKEY
  else
    echo "--> calling create_full_2tensiometer_device_with_dev_addr.sh ${DEFAULT_2TENSIOMETER_NAME} $DEV_ADDR $OPT_DEV_ID $OPT_NO_INIT"
    /home/pi/scripts/create_full_2tensiometer_device_with_dev_addr.sh ${DEFAULT_2TENSIOMETER_NAME} $DEV_ADDR $OPT_DEV_ID $OPT_NO_INIT
  fi
fi
...
echo "Copy conf_block_2tensiometer.yaml into packages/${DEFAULT_2TENSIOMETER_YAML_FILE}"
cp ${HA_HOME}/conf_block_2tensiometer.yaml ${HA_HOME}/packages/${DEFAULT_2TENSIOMETER_YAML_FILE} 
sed -i "s/XXDEV/$DEVICE/g" ${HA_HOME}/packages/${DEFAULT_2TENSIOMETER_YAML_FILE}
sed -i "s/XXNAME/${DEFAULT_2TENSIOMETER_NAME}/g" ${HA_HOME}/packages/${DEFAULT_2TENSIOMETER_YAML_FILE}
sed -i "s/xxname/${DEFAULT_2TENSIOMETER_NAME,,}/g" ${HA_HOME}/packages/${DEFAULT_2TENSIOMETER_YAML_FILE}

echo "Adding into my_default_view.yaml"
cat ${HA_HOME}/view_block_2tensiometer.yaml >> ${HA_HOME}/my_default_view.yaml
sed -i "s/xxname/${DEFAULT_2TENSIOMETER_NAME,,}/g" ${HA_HOME}/my_default_view.yaml 

echo "Copy packages/${DEFAULT_2TENSIOMETER_YAML_FILE} to homeassistant:/config/packages"
docker cp ${HA_HOME}/packages/${DEFAULT_2TENSIOMETER_YAML_FILE} homeassistant:/config/packages
...
/home/pi/scripts/add_to_iiwa_devices.sh $DEVICE $1 2tensiometers
echo "--> set default configuration for $DEVICE in IIWA"
/home/pi/scripts/add_to_iiwa_config.sh $DEVICE 2tensiometers
