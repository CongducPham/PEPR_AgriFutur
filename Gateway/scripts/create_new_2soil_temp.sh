...
# Ex: create_new_2soil_temp.sh 1 --dev-full-addr 260B4515 --appskey BEB72ECC54873DAB0AEE5478ADAB41B7 --nwkskey 262060AA21142DAF8D05902C54F34C58
#
...
# Ex: create_new_2soil_temp.sh 1 --no-init --dev-eui AC1F09FFFE12DA3F
...
if [[ -n $DEV_EUI ]]; then
  ...
  DEFAULT_2SOIL_TEMP_YAML_FILE=${DEFAULT_2SOIL_TEMP_NAME,,}_${DEV_EUI,,}.yaml
elif [[ -n $DEV_FULL_ADDR ]]; then
  echo "Device address: $DEV_FULL_ADDR"
  DEFAULT_2SOIL_TEMP_YAML_FILE=${DEFAULT_2SOIL_TEMP_NAME,,}_${DEV_FULL_ADDR,,}.yaml
else
  DEV_ADDR="$2"
  echo "Device address: $DEV_ADDR"
  DEFAULT_2SOIL_TEMP_YAML_FILE=${DEFAULT_2SOIL_TEMP_NAME,,}_${DEV_ADDR,,}.yaml
fi
...
if [[ -n $DEV_EUI ]]; then
  ...
  echo "--> calling create_full_soil_temperature_device_with_dev_addr ${DEFAULT_2SOIL_TEMP_NAME} $OPT_DEV_ID $OPT_NO_INIT $OPT_DEV_EUI"
  /home/pi/scripts/create_full_soil_temperature_device_with_dev_addr ${DEFAULT_2SOIL_TEMP_NAME} $OPT_DEV_ID $OPT_NO_INIT $OPT_DEV_EUI
else
  ...
  if [[ -n $DEV_FULL_ADDR ]]; then
    echo "--> calling create_full_soil_temperature_device_with_dev_addr ${DEFAULT_2SOIL_TEMP_NAME} $OPT_DEV_ID $OPT_NO_INIT $OPT_DEV_FULL_ADDR $OPT_APPSKEY $OPT_NWKSKEY"
    /home/pi/scripts/create_full_soil_temperature_device_with_dev_addr ${DEFAULT_2SOIL_TEMP_NAME} $OPT_DEV_ID $OPT_NO_INIT $OPT_DEV_FULL_ADDR $OPT_APPSKEY $OPT_NWKSKEY
  else
    echo "--> calling create_full_soil_temperature_device_with_dev_addr ${DEFAULT_2SOIL_TEMP_NAME} $DEV_ADDR $OPT_DEV_ID $OPT_NO_INIT $OPT_DEV_FULL_ADDR $OPT_APPSKEY $OPT_NWKSKEY"
    /home/pi/scripts/create_full_soil_temperature_device_with_dev_addr.sh ${DEFAULT_2SOIL_TEMP_NAME} $DEV_ADDR $OPT_DEV_ID $OPT_NO_INIT $OPT_DEV_FULL_ADDR $OPT_APPSKEY $OPT_NWKSKEY
  fi
fi
