...
if [[ -n $DEV_FULL_ADDR ]]; then
  ...
  echo "--> calling create_full_air_temp_hum_device_with_dev_addr.sh ${DEFAULT_AIR_TEMP_HUM_NAME} $2 $OPT_DEV_ID $OPT_NO_INIT"
  /home/pi/scripts/create_full_air_temp_hum_device_with_dev_addr.sh ${DEFAULT_AIR_TEMP_HUM_NAME} $2 $OPT_DEV_ID $OPT_NO_INIT
fi
