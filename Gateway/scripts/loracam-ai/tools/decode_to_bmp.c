...
void JPEGdecoding(BMPImageStruct* InputImage, BMPImageStruct* OutputImage) {
    ...
    for (int i = 0; i < abs(InputImage->imageVsize); i = i + 8)
        for (int j = 0; j < InputImage->imageHsize; j = j + 8) {
            ...
            for (int u = 0; u < 8; u++)
                for (int v = 0; v < 8; v++) Block[u][v] = Block[u][v] * LuminanceJPEGTable[u][v];
    ...
    while (mqc_numbytes(objet) < packetsize) {
        ...
        for (int x = 0; x < K; x++) {
            ...
            if ((index % 2) == 0) {
                ...
                Block[ZigzagCoordinates[x].row][ZigzagCoordinates[x].col] = 0 - ((index + 1) / 2);
            }
        }
...
void startDecodeImage(char* fileToDecode, FILE* theFile, int Q, int SN, char* paletteFile, int srcAddr, int camId) {
    BMPImageStruct OriginalImage;
    char bmpFile[110], finalBmpFile[110];
    ...
    if (err) {
        ...
        if (SN>=0)
        	snprintf(bmpFile, sizeof(bmpFile), "%s-%d", bmpFile, SN);
        	
        if (srcAddr>=0)
        	snprintf(bmpFile, sizeof(bmpFile), "%s-%04X", bmpFile, srcAddr);       
        	
        if (camId>=0)
        	snprintf(bmpFile, sizeof(bmpFile), "%s-%d", bmpFile, camId);         	 	

        snprintf(finalBmpFile, sizeof(finalBmpFile), "%s-P%d-S%d.bmp", bmpFile, npkt, totalsize);
