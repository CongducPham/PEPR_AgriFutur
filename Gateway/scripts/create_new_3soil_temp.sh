...
# Ex: create_new_3soil_temp.sh 1 --no-init --dev-eui AC1F09FFFE12DA3F
...
DEFAULT_3SOIL_TEMP_NAME="3SOIL_TEMP_${1}"
...
if [[ -n $DEV_EUI ]]; then
  ...
  DEFAULT_3SOIL_TEMP_YAML_FILE=${DEFAULT_3SOIL_TEMP_NAME,,}_${DEV_EUI,,}.yaml
elif [[ -n $DEV_FULL_ADDR ]]; then
  echo "Device address: $DEV_FULL_ADDR"
  DEFAULT_3SOIL_TEMP_YAML_FILE=${DEFAULT_3SOIL_TEMP_NAME,,}_${DEV_FULL_ADDR,,}.yaml
else
  DEV_ADDR="$2"
  echo "Device address: $DEV_ADDR"
  DEFAULT_3SOIL_TEMP_YAML_FILE=${DEFAULT_3SOIL_TEMP_NAME,,}_${DEV_ADDR,,}.yaml
fi

echo "Optional device id: $OPT_DEV_ID"
echo "Optional init value: $OPT_NO_INIT"
echo ${DEFAULT_3SOIL_TEMP_NAME}
echo ${DEFAULT_3SOIL_TEMP_YAML_FILE}
...
if [[ -n $DEV_EUI ]]; then
  ...
  echo "--> calling create_full_soil_temperature_device_with_dev_addr ${DEFAULT_3SOIL_TEMP_NAME} $OPT_DEV_ID $OPT_NO_INIT $OPT_DEV_EUI"
  /home/pi/scripts/create_full_soil_temperature_device_with_dev_addr ${DEFAULT_3SOIL_TEMP_NAME} $OPT_DEV_ID $OPT_NO_INIT $OPT_DEV_EUI
else
  ...
  if [[ -n $DEV_FULL_ADDR ]]; then
    echo "--> calling create_full_soil_temperature_device_with_dev_addr ${DEFAULT_3SOIL_TEMP_NAME} $OPT_DEV_ID $OPT_NO_INIT $OPT_DEV_FULL_ADDR $OPT_APPSKEY $OPT_NWKSKEY"
    /home/pi/scripts/create_full_soil_temperature_device_with_dev_addr ${DEFAULT_3SOIL_TEMP_NAME} $OPT_DEV_ID $OPT_NO_INIT $OPT_DEV_FULL_ADDR $OPT_APPSKEY $OPT_NWKSKEY
  else
    echo "--> calling create_full_soil_temperature_device_with_dev_addr ${DEFAULT_3SOIL_TEMP_NAME} $DEV_ADDR $OPT_DEV_ID $OPT_NO_INIT"
    /home/pi/scripts/create_full_soil_temperature_device_with_dev_addr.sh ${DEFAULT_3SOIL_TEMP_NAME} $DEV_ADDR $OPT_DEV_ID $OPT_NO_INIT
  fi
fi
...
#add the third temperature sensor with lpp channel 11 and name Soil Temperature 3
echo "--> calling create_only_temperature_sensor.sh $DEVICE 11 3"
/home/pi/scripts/create_only_temperature_sensor.sh $DEVICE 11 3
...
echo "--> Add value -99"
curl -X POST "http://localhost/devices/${DEVICE}/sensors/temperatureSensor_10/value" -H "accept: application/json" -H "Authorization: Bearer $TOK" -H  "Content-Type: application/json" -d "{\"value\":-99, \"time\":\"$DATE\"}"

echo "--> Add value -99"
curl -X POST "http://localhost/devices/${DEVICE}/sensors/temperatureSensor_11/value" -H "accept: application/json" -H "Authorization: Bearer $TOK" -H  "Content-Type: application/json" -d "{\"value\":-99, \"time\":\"$DATE\"}"
...
echo "Copy conf_block_3soil_temp.yaml into packages/${DEFAULT_3SOIL_TEMP_YAML_FILE}"
cp ${HA_HOME}/conf_block_3soil_temp.yaml ${HA_HOME}/packages/${DEFAULT_3SOIL_TEMP_YAML_FILE} 
sed -i "s/XXDEV/$DEVICE/g" ${HA_HOME}/packages/${DEFAULT_3SOIL_TEMP_YAML_FILE}
sed -i "s/XXNAME/${DEFAULT_3SOIL_TEMP_NAME}/g" ${HA_HOME}/packages/${DEFAULT_3SOIL_TEMP_YAML_FILE}
sed -i "s/xxname/${DEFAULT_3SOIL_TEMP_NAME,,}/g" ${HA_HOME}/packages/${DEFAULT_3SOIL_TEMP_YAML_FILE}

echo "Adding into my_default_view.yaml"
cat ${HA_HOME}/view_block_3soil_temp.yaml >> ${HA_HOME}/my_default_view.yaml
sed -i "s/xxname/${DEFAULT_3SOIL_TEMP_NAME,,}/g" ${HA_HOME}/my_default_view.yaml 

echo "Copy packages/${DEFAULT_3SOIL_TEMP_YAML_FILE} to homeassistant:/config/packages"
docker cp ${HA_HOME}/packages/${DEFAULT_3SOIL_TEMP_YAML_FILE} homeassistant:/config/packages
