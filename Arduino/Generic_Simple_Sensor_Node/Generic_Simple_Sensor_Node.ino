...
//  #define WITH_RCVW
////////////////////////////
// normal behavior is to invert IQ for RX, the normal behavior at gateway is also to invert its IQ setting, only valid with
// WITH_RCVW
...
// The default address in ABP mode for watermark soil sensor devices is 26011DB1
// if you need a different address for another watermark sensor device, use B2, B3,..., BF instead
unsigned char DevAddr[4] = {0x26, 0x01, 0x1D, 0xB1};
#elif defined AIR_TEMP_HUM_SENSOR
...
#define DHT22_AM2305_DIGITAL_PIN 6
...
// VccCorrection is hardware specific. One should compare the values obtained for and from each specific Arduino board with
// Volt-meter measurements, in order to modify this correction value. First, set VccCorrection to 1.0 for calibration; then get
// the reported Vcc, but only when the device is powered without USB finally set VccCorrection to the ratio of "measured Vcc by
// multimeter" divided by "reported Vcc"
...
// Uncomment to manually test the LOW VOLTAGE MODE, force the VCC_LOW threshold to be reached anyway
...
#define BAT_LOW 3600         // 3.6 V   0% NiMH discharging without solar charge
#else                        // Lithium
...
#endif
#define BAT_MINIMUM 3000  // 3.0 V during radio communication
...
#define WAZISENSE_MOSFET2 7
    ...
    if (recovery_charging) PRINT_CSTSTR("Recovery charging\n");
#endif
    ...
    // WM2
    sensor_ptrs[sensor_index] =
        new watermark("WM2", IS_ANALOG, IS_CONNECTED, low_power_status, (uint8_t)WM2_ANALOG_PIN, (uint8_t)WM2_PWR_PIN1,
                      (uint8_t)WM2_PWR_PIN2 /*use pin trigger as second power pin*/);
    sensor_ptrs[sensor_index]->set_n_sample(NSAMPLE);
...
// ST
    ...
    sensor_ptrs[sensor_index] = new DS18B20("ST1", IS_NOT_ANALOG, IS_CONNECTED, low_power_status, (uint8_t)TEMP_DIGITAL_PIN,
                                            (uint8_t)TEMP_PWR_PIN /*no pin trigger*/);
#else
    sensor_ptrs[sensor_index] = new DS18B20("ST", IS_NOT_ANALOG, IS_CONNECTED, low_power_status, (uint8_t)TEMP_DIGITAL_PIN,
                                            (uint8_t)TEMP_PWR_PIN /*no pin trigger*/);
#endif
    sensor_ptrs[sensor_index]->set_n_sample(NSAMPLE);
...
#if defined TWO_SOIL_TEMP_SENSOR || defined THREE_SOIL_TEMP_SENSOR
    sensor_ptrs[sensor_index] = new DS18B20("ST2", IS_NOT_ANALOG, IS_CONNECTED, low_power_status, (uint8_t)TWO_TEMP_DIGITAL_PIN,
                                            (uint8_t)TEMP_PWR_PIN /*no pin trigger*/);
    sensor_ptrs[sensor_index]->set_n_sample(NSAMPLE);
    ...
    sensor_ptrs[sensor_index]->set_is_power_off_when_inactive(false);
    sensor_index++;
    sensor_ptrs[sensor_index] = new DS18B20("ST3", IS_NOT_ANALOG, IS_CONNECTED, low_power_status, (uint8_t)THREE_TEMP_DIGITAL_PIN,
                                            (uint8_t)TEMP_PWR_PIN /*no pin trigger*/);
    sensor_ptrs[sensor_index]->set_n_sample(NSAMPLE);
    sensor_index++;
#else
    ...
    sensor_ptrs[sensor_index] =
        new DHT22_Humidity((char*)"AH", IS_NOT_ANALOG, IS_CONNECTED, low_power_status, (uint8_t)DHT22_AM2305_DIGITAL_PIN,
                           (uint8_t)DHT22_AM2305_PWR_PIN /*no pin trigger*/);
    sensor_ptrs[sensor_index]->set_n_sample(1);
    // DHT needs about 2s between 2 readings, even if the sensor is already ON
    sensor_ptrs[sensor_index]->set_warmup_time(2100);
    // we don't want to switch on the sensor because the sensor is already ON for reading temperature
...
#endif
#ifdef SHT_HUM_SENSOR
    sensor_ptrs[sensor_index] = new SHT_Humidity((char*)"AH", IS_NOT_ANALOG, IS_CONNECTED, low_power_status, (uint8_t)SHT_SDA_PIN,
                                                 (uint8_t)SHT_PWR_PIN, (uint8_t)SHT_SCL_PIN);
    sensor_ptrs[sensor_index]->set_n_sample(1);
    // we do not need warmup time because the sensor is already ON for reading temperature
    sensor_ptrs[sensor_index]->set_warmup_time(0);
    // we don't want to switch on the sensor because the sensor is already ON for reading temperature
    sensor_ptrs[sensor_index]->set_is_power_on_when_active(false);
    sensor_index++;
#endif
...
*******************************************************************************************************/
#endif  // endif if defined RAW_LORA && defined WITH_SPI_COMMANDS
...
#if defined WITH_AES && not defined LORAWAN
...
#ifdef MONITOR_BAT_VOLTAGE
//////////////////////////////////////////////////////////////
// Function returning instantaneous voltage as read by Arduino, in mV
...
////////////////////////////////////////////////////////////////
// return percent (can be more than 100% if voltage is too high)
...
// the soil temperature sensor
...
#if defined DHT22_AM2305_HUM_SENSOR || defined SHT_HUM_SENSOR
            if (strncmp(sensor_ptrs[i]->get_nomenclature(), "AH", 2) == 0) {
    ...
    r_size = sprintf((char*)message, final_str);
...
///////////////////////////////////
    ...
    pl = local_aes_lorawan_create_pkt(lpp.getBuffer(), lpp.getSize(), 0);
#else
pl = local_aes_lorawan_create_pkt(message, pl, 0);
#endif
#else
...
// we use raw format for LoRaWAN
...
#endif  // TEST_LOW_POWER_NO_TRANSMIT
    {
        ...
        // low voltage, so last_vcc = vcc.Read_Volts(); would not be executed
    ...
    if (RXPacketL) {
        ...
        sprintf((char*)print_buff, "^p%d,%d,%d,%d,%d,%d,%d\n", LT.readRXDestination(), LT.readRXPacketType(), LT.readRXSource(),
                LT.readRXSeqNo(), RXPacketL, LT.readPacketSNR(), LT.readPacketRSSI());
        ...
        for (i = 0; i < RXPacketL; i++) {
            if (message[i] < 16) PRINT_CSTSTR("0");
        ...
        if (i >= 0 && message[i] == '/' && message[i + 1] == '@') {
            ...
            switch ((char)cmdstr[i]) {
                ...
                case 'I':
                    ...
                    if (cmdValue < 1) cmdValue = idlePeriodInMin;
                    // idlePeriodInMin
